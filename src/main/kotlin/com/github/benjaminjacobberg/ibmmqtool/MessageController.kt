...
class MessageController(private val messageService: MessageService) {
    ...
    fun sendMessage(sendMessageDTO: SendMessageDTO) {
        val message: Message = Message(sendMessageDTO.body)
        val connectionInformation: ConnectionInformation = ConnectionInformation(
                host = sendMessageDTO.host,
                port = sendMessageDTO.port,
                channel = sendMessageDTO.channel,
                qm = sendMessageDTO.qm,
                userId = sendMessageDTO.userId,
                password = sendMessageDTO.password,
    ...
    fun getMessage(@PathVariable(value = "size", required = false) size: Int = 100,
                   @PathVariable(value = "host") host: String,
                   @PathVariable(value = "port") port: Int,
                   @PathVariable(value = "channel") channel: String,
                   @PathVariable(value = "qm") qm: String,
                   @PathVariable(value = "userId") userId: String,
                   @PathVariable(value = "password") password: String,
                   @PathVariable(value = "queue") queue: String): List<String> {
        val connectionInformation: ConnectionInformation = ConnectionInformation(host = host,
                                                                                 port = port,
                                                                                 channel = channel,
                                                                                 qm = qm,
                                                                                 userId = userId,
